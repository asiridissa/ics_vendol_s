...
DEFINE VARIABLE filBillNo AS INT64 FORMAT ">>>>>>>9":U INITIAL 0 
     LABEL "Bill No" 
     VIEW-AS FILL-IN 
     SIZE 8.86 BY .88
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brwBill C-Win _STRUCTURED
  QUERY brwBill NO-LOCK DISPLAY
      bills.BillNo COLUMN-LABEL "Bill No" FORMAT ">>>>>>>9":U
...
"bills.BillNo" "Bill No" ">>>>>>>9" "int64" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
